...
class Blockchain():
    ...
    def run(self,port):
        pass
        self.port = port
        genesis_block = Block("1",[100,100,100,100],"0.0.0.0 0",'random')
    ...
    def update_blockchain_zkp(self,block):
        self.UPDATED = 0
        while self.UPDATING == True: pass #有別的執行續正在更新
        self.UPDATING = True
        verify = self.verify_zkp(block)
        if(verify == True):
            #print("[blockchain.update_blockchain]self.UPDATE = 1")
            self.UPDATE = 1
            for _ in self.blockqueue:
    ...
    def verify(self,block): #input:block, output:True(bool),(str)
        ...
        if block.info.pos == 0:
            self.difficulty = (int(block.info.data) % 2) + 1 
            print("[test_dif]",self.difficulty)
            print("[test_dif_hash]",block.get_blockhash())
            if(block.get_blockhash()[0:self.difficulty] != '0' * self.difficulty):
                print("current hash is fake")
                print("[test_dif_nonce]",block.nonce)
    ...
    def mine(self):
        if(len(self.blockqueue) >= 1):
            ...
            self.difficulty = (int(self.blockqueue[0].info.data) % 2) + 1
            self.UPDATE = 0
    ...
    def write_log_check(self,port = 0,final_time = "",miner = ""): 
        ...
        msg += "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n"
        msg += "str_data:"
        msg_hex = hex(int(self.chain[-1].info.data))
        msg_str = self.hex_to_string(msg_hex)
    ...
    def write_log_blockqueue(self,port = 0,debug = "writelog"): 

        if port == 5000:
            f = open("blockqueue1.log",'w')
        elif port == 5001:
            f = open("blockqueue2.log",'w')
        elif port == 5002:
            f = open("blockqueue3.log",'w')
        elif port == 5003:
            f = open("blockqueue4.log",'w')
