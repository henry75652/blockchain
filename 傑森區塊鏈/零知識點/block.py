...
class Block():
    ...
    def conv_block_to_str_zpk(self,en_flag,fake_flag):
        str_block = str()
        #inf = str(self.information.data)
        if en_flag == 0:
            if fake_flag == 0:
                str_block += str(self.id) \
                    + "\\" + str(self.timestamp) \
                    + "\\" + str(self.prev_hash) \
                    + "\\" + self.AES0.encrypt(str(int(self.info.data) * self.nonce) + "//" + str(self.info.pos_trust) + "//" + str(self.info.pos) + "//" + str(self.info.pos_miner)) \
                    + "\\" + str(self.source) \
                ...
                str_block += str(self.id) \
                    + "\\" + str(self.timestamp) \
                    + "\\" + str(self.prev_hash) \
                    + "\\" + self.AES1.encrypt(str(int(self.info.data) * self.nonce) + "//" + str(self.info.pos_trust) + "//" + str(self.info.pos) + "//" + str(self.info.pos_miner)) \
                    + "\\" + str(self.source) \
                #print("[before_send_en]",self.AES1.encrypt(str(self.information.data)),"\n")
        elif en_flag == 1:
            str_block += str(self.id) \
                + "\\" + str(self.timestamp) \
                + "\\" + str(self.prev_hash) \
                + "\\" + str(int(self.info.data) * self.nonce) + "//" + str(self.info.pos_trust) + "//" + str(self.info.pos) + "//" + str(self.info.pos_miner) \
                + "\\" + str(self.source) \
                #print("[own_no_en]",str(self.information.data),"\n")
        return str_block
    ...
    def conv_str_to_block_zpk(self,sData):  #return 
        # 56\Mon Feb 21 14:29:13 2022\177\00079bcd42436498efda60b225436ec25c8db797be85775b0271bbba4a776ddf\Hello\0.0.0.0
        list_block = sData.split('\n')  #['Tue Dec 14 00:21:29 2021\\hello\\0\\0\\172.30.5.59:5000', '']
        #print("[conv_str_to_block]\nsData\n",sData,"\nlist_block\n",list_block)
        list_block = list_block[0]          #  id    timestamp                   nonce  prev_hash                                                           information source
        ...
        #print("[test2_block.py]",self.source)
        list_info_total = list_block[3].split('//')
        self.info.data = list_info_total[0]
        self.info.pos_trust = commitment_cal().sequence_info(list_info_total[1])
        #print("test_block.py_pass_2")
        self.info.pos = int(list_info_total[2])
    ...
    def is_same_as(self,block):
        ...
        if ("timestamp" in result) and ("source" in result):
    ...
    def get_blockhash_zpk(self):
        new_block = self.duplication()
        new_block.id = 0
        return hashlib.sha256(new_block.conv_block_to_str_zpk(1,0).encode()).hexdigest()
