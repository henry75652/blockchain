...
class Blockchain():
    ...
    def run(self,port):
        pass
        self.port = port
        genesis_block = Block("this is genesis block",[100,100,100,100])
        #self.update_blockchain(genesis_block) 6.27
        genesis_block.timestamp = float(0)
        self.chain.append(genesis_block)
        
    def update_blockchain(self,block,mine_time = ""):
        ...
        if(verify == True):
            ...
            self.write_log_check(port = 0,final_time = mine_time)
        else:
    ...
    def mine(self):
        if(len(self.blockqueue) >= 1):
            ...
            self.update_blockchain(block,mine_time = str(round(time.time(),2)))
            return block

    def write_log_check(self,port = 0,final_time = ""): 
